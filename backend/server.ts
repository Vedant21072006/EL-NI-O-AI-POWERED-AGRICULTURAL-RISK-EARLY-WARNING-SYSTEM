...
import { handleKrishiChat } from './chatEngine';
import { MAHARASHTRA_DISTRICTS, HISTORICAL_RECORDS } from './data/maharashtraDistricts';
import { CropStage, IrrigationType } from './types';
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'ok',
    service: 'KrishiAlert El Niño Risk Engine (Standalone Backend)',
...
// Root welcome route for backend health inspection
app.get('/', (req, res) => {
  res.json({
    name: 'KrishiAlert API',
    description: 'El Niño Agricultural Risk & Early Warning Backend Service',
    status: 'running',
    docs: '/api/health'
  });
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🌾 KrishiAlert Backend API running on port ${PORT}`);
});
